	...
	<div class="container">
		...
		<div class="card">
			<div class="form-group">
				<label>비품 종류</label> <select id="category" onchange="updateItems()">
			...
			<div class="form-group">
				<label>비품 선택</label> <select id="item" onchange="showItemInfo()">
			...
			<div class="form-group">
				<label>수량 <span id="availBadge"></span></label> <select
					id="quantity" disabled>
					<option value="">-- 비품을 먼저 선택하세요 --</option>
				</select>
			</div>
			<div class="form-group">
				<label>대여 시작일</label> <input type="date" id="startDate">
			</div>
			<div class="form-group">
				<label>반납 예정일</label> <input type="date" id="endDate"
			...
			<button class="btn-submit" onclick="submitRequest()">대여 요청
				제출</button>
		</div>
	</div>
	<script>
const items = {
  ...
  '노트북': [
    { name: 'LG그램 15인치', serial: 'NTB-001', total: 5, using: 2, report: 0 },
    { name: '삼성 갤럭시북 Pro', serial: 'NTB-002', total: 5, using: 1, report: 0 },
    { name: 'Dell XPS 15', serial: 'NTB-003', total: 4, using: 1, report: 1 }
  ],
  '카메라': [
  ...
  '보조배터리': [
    { name: 'Anker 10000mAh', serial: 'BAT-001', total: 8, using: 3, report: 1 },
    { name: 'Anker 30000mAh', serial: 'BAT-002', total: 8, using: 2, report: 0 }
  ],
  '빔프로젝터': [
    { name: 'Epson EB-X49', serial: 'PRJ-001', total: 2, using: 1, report: 0 },
    { name: 'BenQ MH560', serial: 'PRJ-002', total: 2, using: 0, report: 1 }
  ]
};
